...
class ListaFinalSupabaseUser extends BaseAuthUser {
  ListaFinalSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> listaFinalSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ListaFinalSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
